    ...
    async function checkOnboarding() {
      ...
      if (user) {
        ...
        try {
          ...
          const hasOnboardingData = userData?.target_profession && userData?.grade;
          const isOnboardingCompleted = userData?.onboarding_completed || hasOnboardingData;
          
          if (userData?.created_at && !isOnboardingCompleted) {
            const createdDate = new Date(userData.created_at);
            const today = new Date();
            // Sadece tarih kısmını karşılaştır (saat farkı olmasın)
            const isNewUser = createdDate.toDateString() === today.toDateString();
